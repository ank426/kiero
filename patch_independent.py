def _run(
    ...
    # If the input is a CBZ, unpack it and recurse.
    ...
    # If the input is a directory (either originally or extracted above) and output is CBZ,
    # process into a temp dir and then pack it.
    ...
    # Base case 1: Directory to Directory
...
def _detect(
    ...
    # Detect only reads the input, so we just extract if it's a CBZ.
    if _is_cbz(input_path):
        import shutil
...
def _inpaint(input_path: Path, output_path: Path, mask: Path, device: str | None):
    ...
    # If the input is a directory and output is CBZ, process into a temp dir and then pack it.
    if input_path.is_dir() and _is_cbz(output_path):
        import shutil
        import tempfile
